...
(function () {
  ...
  function connect() {
    ...
    client.from(TABLE).select('key,data').then(function (res) {
      ...
      // Cloud gewinnt wo vorhanden — lokale Keys ohne Cloud-Pendant bleiben erhalten
      // (verhindert Datenverlust bei Push-Fehlern, z.B. Verfügbarkeit)
      var cloudKeys = {};
      (res.data || []).forEach(function (row) {
        rawSet(row.key, typeof row.data === 'string' ? row.data : JSON.stringify(row.data));
        cloudKeys[row.key] = true;
      });
      // Lokale Keys die Cloud NICHT kennt → sofort pushen (könnte verlorener Push sein)
